...
document.addEventListener('DOMContentLoaded', function () {
	...
	form.addEventListener('submit', async function (e) {
		...
		try {
			...
			// Если поле "languages" было множественным выбором — добавляем все значения
			const langSelect = form.querySelector('[name="languages[]"]')
			if (langSelect) {
				formData.delete('languages[]') // удаляем старые значения
				for (const option of langSelect.selectedOptions) {
			...
			// Получаем URL из action или текущего пути
			const formAction = form.getAttribute('action') || '/project/index.php?q=form'
	...
	function validateForm() {
		...
		if (!phone.value.trim()) errors.phone = 'Укажите телефон'
		if (!email.value.trim()) errors.email = 'Укажите Email'
		if (!birth_day.value || !birth_month.value || !birth_year.value) errors.birth_date = 'Укажите полную дату рождения'
		if (!gender) errors.gender = 'Выберите пол'
		if (selectedLangs.length === 0) errors.languages = 'Выберите хотя бы один язык' // ❌ было 'lang', ✅ стало 'languages'
		if (!biography.value.trim()) errors.biography = 'Расскажите о себе'
		if (!agreement?.checked) errors.agreement = 'Необходимо согласие'
